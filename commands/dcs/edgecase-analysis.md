...
- $1 がない場合、「引数に要件名または分析対象を指定してください（例: /mille:dcs:kairo-edgecase-analysis ユーザー認証システム）」と言って終了する
  ...
  - 存在しない場合:
    ...
    - ユーザーの選択に応じて分岐:
      ...
      - "要件定義書を作成してから分析" を選択した場合:
        - 「要件定義書の作成が必要です。先に `/mille:kairo-requirements {要件名}` を実行してください」とメッセージを表示
        - 終了する
