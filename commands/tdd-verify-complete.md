...
- **テスト実行時間の確認**:
  ...
  - 遅いテストがある場合は以下を推奨として記録:
    ...
    2. `/mille:test-optimization-patterns` でパターンを確認
    3. 次のTDDサイクルで段階的に改善
    ```
...
- 要件網羅率: 100%
- 今回のタスクの要件充実度は完全達成
- スコープ外の問題は別途 /mille:auto-debug で対応
...
次のお勧めステップ: `/mille:tdd-cycle` で次のTDDサイクルを開始します。
</complete_output>
...
🔧 **推奨対応**: `/mille:auto-debug` を実行してスコープ外の失敗テストを修正してください。
