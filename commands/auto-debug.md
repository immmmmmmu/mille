...
3. **全テストケースの確認（簡潔モード）**
   ...
   - テストファイルごとにグループ化
   - **タイムアウト発生時**: /mille:timeout-fix コマンドを subagent（general-purpose）で実行
...
4. **ビルドエラーの判定**
   ...
   - **ビルドエラーの場合**:
     - /mille:build-fix コマンドを subagent（general-purpose）で実行（test_command と error_output を渡す）
...
2. **該当テストファイルのみを実行して詳細確認**（ここでのみ--verbose使用）
   ...
   - **タイムアウト発生時**: /mille:timeout-fix コマンドを subagent で実行（test_file を指定）
     - 成功 → テスト再実行
...
3. **flaky test の検知**
   ...
   - 2回目に成功した場合: **flaky test と判定**
     - /mille:flaky-fix コマンドを subagent（general-purpose）で実行（test_file と error_output を渡す）
...
6. **環境・依存関係問題の委譲**
   - エラー分類で「⚙️ 設定・環境問題」または「📦 依存関係問題」と判定された場合:
     - /mille:env-fix コマンドを subagent（general-purpose）で実行（test_command, test_file, error_output, error_classification を渡す）
...
テスト実行速度の改善を推奨します:
1. `/mille:dcs:test-performance-analysis` で詳細分析
2. `/mille:test-optimization-patterns` でリファクタリングパターンを確認
```
...
MUST: ビルドエラー検出時はまず /mille:build-fix に委譲し、失敗時のみレポート
MUST: flaky test検出時はまず /mille:flaky-fix に委譲し、失敗時のみ除外・レポート
MUST: 環境・依存関係問題はまず /mille:env-fix に委譲し、失敗時のみ保留・レポート
MUST: タイムアウト発生時はまず /mille:timeout-fix に委譲し、失敗時のみ分離またはレポート
