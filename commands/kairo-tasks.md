...
次のステップ表示: 「次のお勧めステップ: `/mille:kairo-implement` でタスクを実装します。特定のタスクを実装する場合は `/mille:kairo-implement TASK-0001` のように指定してください。」
...
- 次のステップ表示:
  ...
  次のお勧めステップ: `/mille:kairo-implement` でタスクを実装します。
  ```
...
### TDDタスク
1. `/mille:tdd-requirements` - 詳細要件定義
2. `/mille:tdd-testcases` - テストケース作成
3. `/mille:tdd-red` - テスト実装（失敗）
4. `/mille:tdd-green` - 最小実装
5. `/mille:tdd-refactor` - リファクタリング
6. `/mille:tdd-verify-complete` - 品質確認

### DIRECTタスク
1. `/mille:direct-setup` - 直接実装・設定
2. `/mille:direct-verify` - 動作確認・品質確認
...
```
/mille:kairo-tasks ユーザー認証システム --task
```
...
タスクを実装するには:
- 全タスク順番に実装: `/mille:kairo-implement`
- 特定タスクを実装: `/mille:kairo-implement TASK-0001`
</overview_template>
...
1. `/mille:tdd-requirements TASK-{番号}` - 詳細要件定義
2. `/mille:tdd-testcases` - テストケース作成
3. `/mille:tdd-red` - テスト実装（失敗）
4. `/mille:tdd-green` - 最小実装
5. `/mille:tdd-refactor` - リファクタリング
