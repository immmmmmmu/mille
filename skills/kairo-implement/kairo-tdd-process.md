...
```
@Task tool (subagent_type: general-purpose, model: {{noteTaskName}}) /mille:tdd-tasknote {{要件名}} {{TASK-ID}}
```
...
```
@Task tool (subagent_type: general-purpose, model: {{thinkTaskName}}) /mille:tdd-requirements {{要件名}} {{TASK-ID}}
```
...
```
@Task tool (subagent_type: general-purpose, model: {{thinkTaskName}}) /mille:tdd-testcases {{要件名}} {{TASK-ID}}
```
...
```
@Task tool (subagent_type: general-purpose, model: {{tddTaskName}}) /mille:tdd-red {{要件名}} {{TASK-ID}}
```
...
```
@Task tool (subagent_type: general-purpose, model: {{tddTaskName}}) /mille:tdd-green {{要件名}} {{TASK-ID}}
```
...
```
@Task tool (subagent_type: general-purpose, model: {{tddTaskName}}) /mille:tdd-refactor {{要件名}} {{TASK-ID}}
```
...
```
@Task tool (subagent_type: general-purpose, model: {{tddTaskName}}) /mille:tdd-verify-complete {{要件名}} {{TASK-ID}}
```

判定結果に応じた処理:
- **テストケース不足**: 不足内容を明示した上で step-d（tdd-red）に戻り、テストケースを追加
- **実装不足（テストケースは十分）**: 失敗テストを明示した上で step-e（tdd-green）に戻り、実装を追加
- **スコープ外のみ失敗**: step-h へ進む + ユーザーに `/mille:auto-debug` 推奨を通知
- **OK**: step-h へ進む
